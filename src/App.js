...
const progressBarT = document.querySelector(".progress-inner-true")
const progressBarF = document.querySelector(".progress-inner-false")
...
function App() {
  ...
  function handleAnswer(isCorrect) {
    if (isCorrect) {
      progressBarT.style.transform = `scaleX(${score/4})`
      setScore(score + 1);
      const nextQuestion = currentQuestion + 1;
      ...
      setfScore(fscore+1);
      const nextQuestion = currentQuestion + 1;
      progressBarF.style.transform = `scaleX(${fscore/4})`
    if (nextQuestion < questions.length) {
      setCurrentQuestion(nextQuestion);
    }
